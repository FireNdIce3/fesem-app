...
admin.initializeApp({
  ...
  storageBucket:
    "https://abheyshahpersonalproject-default-rtdb.firebaseio.com/", // Replace with your Firebase Storage bucket name
});
...
app.post("/admin/delete", jsonParser, async (req, res) => {
  ...
  return transporter.sendMail(mailOptions6, async function (error, info) {
    if (error) {
      console.log(error);
    } else {
      console.log("Email sent: " + info.response);
...
app.post("/fesem/forget", jsonParser, async function (req, res) {
  ...
  if (result.length === 0) {
    ...
    return transporter.sendMail(mailOptions5, function (error, info) {
      if (error) {
        console.log(error);
      } else {
        console.log("Email sent: " + info.response);
...
app.post("/addInvoice", jsonParser, async function (req, res) {
